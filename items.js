...
const ITEM_DB = { // Food, Toy, Quest, Misc, Powerup, Trophy, Valuable ===
    ...
    102: {id: 102, name: "Quacker", emoji: "🦆", type: "Toy", stackable: true, maxStack: 999, rarity: "Common", price: 1, combinable: false, usable: false, description: "Quack!, Collect them all!"},
    ...
    501: {id: 501, name: "Four leaf clover", emoji: "🍀", type: "Trophy", stackable: false, maxStack: 1, rarity: "Legendary", price: 10, combinable: false, usable: false, description: "A four leaf clover!, Lucky you!"},
    ...
    // Notifications
    996: {id: 996, name: "Cant Combine!", emoji: "❌", type: "", stackable: false, maxStack: 1, rarity: "Notif", combinable: false, usable: false, description: "Cant combine these items."},
    997: {id: 997, name: "Coins", emoji: "💰", type: "", stackable: false, maxStack: 1, rarity: "Notif", combinable: false, usable: false, description: "its money"},
    998: {id: 998, name: "Not enough coins!", emoji: "❗", type: "", stackable: false, maxStack: 1, rarity: "Notif", combinable: false, usable: false, description: "You don't have enough coins."},
    999: {id: 999, name: "Inventory Full!", emoji: "❗", type: "", stackable: false, maxStack: 1, rarity: "Notif", combinable: false, usable: false, description: "No space left in inventory."},
...
function rollLoot(poolIds) {
  const entries = poolIds.map(id => ({
    id,
    weight: rarityWeights[ ITEM_DB[id]?.rarity ] ?? 1
  }));
  const pick = pickWeighted(entries);
  return pick?.id; // return id
}

// Define loot pools by name
const LOOT_POOLS = {
  Food:     [1, 2, 3, 4, 5, 6, 7, 8],                   // Food items
  Toy:      [100, 101, 102, 103],                      // Toy items
  Quest:    [200, 201, 202],                          // Quest items
  Misc:     [300, 301, 302, 303, 304, 305, 306],     // Misc items
  Powerup:  [400, 401, 402],                        // Powerup items
  Trophy:   [500, 501, 502, 503, 504, 505],        // Trophy items
  Valuable: [600, 601, 602, 603, 604],            // Valuable items
};
...
function combineItem(index) {
  ...
  const combinations = [
    ...
    // {
    //   check: () => item.id === X && ...,
    //   combine: () => { removeItemsById(...); addItemById(...); showNotif(...); }
    // },
  ];
